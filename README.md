...
- **🏃 退场动画**：任何退出方式（菜单/托盘/宿主看门狗）都会先喊一声拜拜，然后**加速蹦跳着冲向最近的屏幕边缘**，完全出屏后才消失（多显示器按所在屏幕计算方向）
...
- **叉窗口 = 全家下班**：通过 `启动桌面版.bat` 启动时，后台会挂载**会话管家**（`pet\watch-session.ps1`）监视 Harness 桌面窗口——叉掉窗口后约 6 秒内自动关停 `dsh web` 服务，桌宠看门狗随之退场（直接 `dsh web` 启动时则由桌宠的宿主看门狗兜底）
...
- **在 Harness 设置中查看**：打开 Harness 界面 → 设置 → 插件 → **插件列表**，搜索 `dsh-desktop-pet` 即可看到桌宠插件的启用状态标签（该页签为只读盘点视图，Harness 官方插件同样没有开关按钮，开关统一在配置文件层）
